class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        '''
        Find all the cells that can reach the pacific in a set
        Find all the cells that can reach the atlantic in a set
        Comparison needs to check if neighboring cell can flow to itself
        Start from edges and bfs outwards
